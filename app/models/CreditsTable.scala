...
class CreditsTable(tag:Tag) extends Table[Credit](tag,"CREDITS") with CustomConverters {
  ...
  def programme = column[String]("CRED_PROG_ID")
  def generation = column[Int]("GENERATION")
  def role = column[String]("CRED_ROLE")
  def name = column[String]("CRED_NAME")

  def programmeFK = foreignKey("FK_CRED_PROGRAMME",programme, programmes)(_.uniqueId, onUpdate=ForeignKeyAction.Restrict, onDelete = ForeignKeyAction.Cascade)
